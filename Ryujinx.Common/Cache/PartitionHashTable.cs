...
namespace Ryujinx.Common.Cache
{
    class PartitionHashTable<T>
    {
        ...
        public SearchResult TryFindItem(ref SmartDataAccessor dataAccessor, int size, ref T item)
        {
            if (_count == 0)
            {
            ...
            ReadOnlySpan<byte> data = dataAccessor.GetSpanAndHash(size, out uint dataHash);
